...
class DailyCardInteraction(db.Model):
    """每日卡片互动记录 — 记录用户是否已读/已回答"""
    __tablename__ = 'daily_card_interactions'

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    card_date = db.Column(db.String(10), nullable=False)  # YYYY-MM-DD
    has_read = db.Column(db.Boolean, default=False)
    has_answered = db.Column(db.Boolean, default=False)
    answer_text = db.Column(db.Text, nullable=True)  # 用户对卡片的思考/回答
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    __table_args__ = (db.UniqueConstraint('user_id', 'card_date', name='unique_user_card_date'),)
